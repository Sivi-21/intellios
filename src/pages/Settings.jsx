...
import { EngineTheme, SynthHumType } from '../context/AppContext';
...
export const Settings: React.FC = () => {
  ...
  // Profile icon selections helper
  ...
  const handleThemeChange = (selectedTheme: EngineTheme) => {
    setEngineTheme(selectedTheme);
    const preset = THEME_PRESETS[selectedTheme];
    // Trigger sound effects corresponding to compilation success
    if (audioFeedback) {
      ...
      try {
        ...
        if (AudioContextClass) {
          ...
          gain.gain.linearRampToValueAtTime(0.001, tCtx.currentTime + 0.1);
          osc.connect(gain);
  ...
  const handleHumChange = (type: SynthHumType) => {
    playSfx('click');
    setSynthHum(type);
  ...
  const handleFeedbackToggle = () => {
    ...
    // Quick click sound if toggled ON
    if (nextVal) {
      try {
        const AudioContextClass = window.AudioContext || (window as any).webkitAudioContext;
        if (AudioContextClass) {
          ...
          gain.gain.exponentialRampToValueAtTime(0.001, tCtx.currentTime + 0.04);
          osc.connect(gain);
          gain.connect(tCtx.destination);
          osc.start();
  ...
  const ambientHumOptions: Array<{ id: SynthHumType; label: string; desc: string }> = [
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="p-6 bg-zinc-950/60 border border-zinc-900 rounded-2xl space-y-4 shadow-xl relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-2">
            {(Object.keys(THEME_PRESETS) as EngineTheme[]).map((themeKey) => {
              ...
              return (
                <button
                  ...
                  className={`p-3.5 rounded-xl border text-left transition-all duration-200 select-none cursor-pointer flex flex-col justify-between h-24 ${
                    isActive 
                      ? 'bg-zinc-900 text-white shadow-md' 
                      : 'bg-zinc-950/40 hover:bg-zinc-900/40 border-zinc-950 hover:border-zinc-800 text-zinc-400 hover:text-zinc-200'
                  }`}
                  style={{
                    borderColor: isActive ? `rgba(${preset.rawRGB}, 0.4)` : undefined,
                    boxShadow: isActive ? `0 0 10px rgba(${preset.rawRGB}, 0.08)` : undefined
                  }}
                >
                  <div className="flex items-center justify-between w-full">
                    ...
                    {isActive && (
                      <span className="w-2 h-2 rounded-full" style={{ backgroundColor: `rgb(${preset.rawRGB})` }} />
                    )}
                  </div>
                ...
                {ambientHumOptions.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-2 px-3 text-[11px] font-sans rounded-xl border text-left cursor-pointer transition-all duration-150 flex flex-col justify-between min-h-[58px] ${
                        isActive 
                          ? 'bg-zinc-900 text-white' 
                          : 'bg-zinc-950/45 hover:bg-zinc-900/35 border-transparent hover:border-zinc-850 text-zinc-400 hover:text-zinc-200'
                      }`}
                      style={{
                        borderColor: isActive ? `rgba(${themeColors.rawRGB}, 0.25)` : undefined,
                        boxShadow: isActive ? `0 0 10px rgba(${themeColors.rawRGB}, 0.05)` : undefined
                      }}
                    >
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`w-11 h-6 rounded-full transition-all relative ${
                  audioFeedback ? 'bg-cyan-500' : 'bg-zinc-800'
                }`}
                style={{
                  backgroundColor: audioFeedback ? `rgb(${themeColors.rawRGB})` : undefined
                }}
              >
                <div className={`w-4.5 h-4.5 bg-neutral-950 rounded-full absolute top-0.75 transition-all ${
                  audioFeedback ? 'left-5.5' : 'left-0.75'
                }`} />
              </button>
            </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {availableAvatars.map(av => (
                    <button
                      ...
                      className={`w-7.5 h-7.5 rounded-lg flex items-center justify-center text-sm transition-all cursor-pointer ${
                        profileAvatar === av 
                          ? 'bg-zinc-800 text-white border border-zinc-700 font-bold scale-105' 
                          : 'bg-zinc-900/60 hover:bg-zinc-900 border border-zinc-900/60 text-zinc-400 hover:text-zinc-200'
                      }`}
                    >
                      {av}
                    </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="p-6 bg-zinc-950/60 border border-zinc-900 rounded-2xl space-y-4 shadow-xl relative overflow-hidden">
            ...
            <div className="space-y-4 pt-1">
              ...
              <div className="p-3 rounded-xl bg-zinc-900 border border-zinc-905 flex gap-3 text-xs leading-normal"
                style={{
            ...
            <div className="pt-2 space-y-4.5">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`w-11 h-6 rounded-full transition-all relative ${
                    autoSaveEnabled ? 'bg-cyan-500' : 'bg-zinc-800'
                  }`}
                  style={{
                    backgroundColor: autoSaveEnabled ? `rgb(${themeColors.rawRGB})` : undefined
                  }}
                >
                  <div className={`w-4.5 h-4.5 bg-neutral-950 rounded-full absolute top-0.75 transition-all ${
                    autoSaveEnabled ? 'left-5.5' : 'left-0.75'
                  }`} />
                </button>
              </div>
