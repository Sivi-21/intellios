...
const Column: React.FC<ColumnProps> = ({ title, count, colorClass, children }) => {
  return (
    <div className="flex flex-col flex-1 min-w-[250px] bg-zinc-950/40 rounded-2xl border border-zinc-900 overflow-hidden pb-4">
      {/* Column Title bar */}
...
export const Tasks: React.FC = () => {
  ...
  const cycleStatus = (id: string, current: TaskStatus, direction: 'forward' | 'backward') => {
    const sequence: TaskStatus[] = ['backlog', 'todo', 'in_progress', 'completed'];
    ...
    <div className="space-y-6 select-none relative">
      ...
      <div className="flex flex-col lg:flex-row gap-4.5 overflow-x-auto pb-4 items-stretch h-[calc(100vh-14rem)]">
        {columns.map(col => {
          ...
          return (
            <Column key={col.status} title={col.title} count={colTasks.length} colorClass={col.color}>
              {colTasks.length === 0 ? (
                <div className="h-32 border border-dashed border-zinc-900 flex flex-col items-center justify-center text-center p-4 rounded-xl font-sans text-zinc-600 gap-1 mt-1 opacity-70">
