...
const CodeSnippetViewer = ({ code, language, filePath }) => {
  ...
  return <div className="my-4.5 rounded-xl bg-zinc-950 border border-zinc-850 overflow-hidden font-mono text-xs shadow-lg shadow-zinc-950/50">
...
export const AIChat = () => {
  ...
  return <div className="flex flex-col h-[calc(100vh-8.5rem)] rounded-2xl bg-zinc-950/60 border border-zinc-900 overflow-hidden shadow-2xl relative select-none">
  ...
  >
    ...
    className={`w-full p-2 text-left rounded-lg text-xs transition-colors flex flex-col cursor-pointer ${activeModel === m.name ? "bg-zinc-900 text-zinc-100" : "hover:bg-zinc-900/50 text-zinc-400 hover:text-zinc-200"}`}
  >
  ...
  >
    ...
    /* Main Scrollable Messages Box */
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5 bg-zinc-950/20">
        ...
        {chatMessages.map((msg) => <motion.div
    key={msg.id}
  ...
  >
    ...
    /* Content box */
            ...
            <div className={`rounded-xl p-4 text-xs leading-relaxed max-w-[85%] border shadow-xl ${msg.sender === "user" ? "bg-zinc-900 text-zinc-100 border-zinc-800" : "bg-zinc-900/35 text-zinc-300 border-zinc-900/60 font-sans backdrop-blur cursor-text select-text select-all"}`}>
              {
    ...
    dangerouslySetInnerHTML={{
      __html: msg.text.replace(/\*\*(.*?)\*\*/g, '<strong class="text-white text-medium">$1</strong>').replace(/\*(.*?)\*/g, '<em class="text-zinc-100">$1</em>').replace(/`(.*?)`/g, '<code class="bg-zinc-950 px-1 py-0.5 rounded font-mono text-cyan-400">$1</code>')
    }}
  />
    ...
    /* Nested codeblock if present */
              ...
              {msg.codeSnippet && <CodeSnippetViewer
  ...
  >
    ...
    /* Recommended macro prompts floating footer bar */
      ...
      <div className="px-5 py-2.5 bg-zinc-950/40 border-t border-zinc-900 flex flex-wrap gap-2 overflow-x-auto whitespace-nowrap scrollbar-none select-none">
        {macroPrompts.map((macro, i) => <button
    key={i}
