...
const SidebarItem = ({
  ...
  return <button
    onClick={onClick}
    style={{
      boxShadow: active ? activeColorShadow : void 0,
      border: active ? activeBorder : "1px solid transparent"
    }}
    className={`relative w-full flex items-center gap-3 p-3 rounded-lg text-sm transition-all duration-200 group overflow-hidden ${active ? "bg-zinc-800/60 text-white" : "text-zinc-400 hover:text-zinc-100 hover:bg-zinc-900/50"}`}
  >
      {
  ...
  >
          <span>{label}</span>
          {badge && <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded border ${badgeColor || theme.badgeAccent}`}>
              {badge}
            </span>}
        </motion.div>}
    ...
    /* Tooltip for collapsed mode */
      ...
      {isCollapsed && <div className="absolute left-14 invisible group-hover:visible font-sans bg-zinc-950 text-zinc-100 text-xs px-2.5 py-1.5 rounded-md border border-zinc-800 opacity-0 group-hover:opacity-100 translate-x-2 group-hover:translate-x-0 transition-all duration-200 shadow-xl z-55 whitespace-nowrap pointer-events-none">
          {label}
...
export const Sidebar = () => {
  ...
  const navItems = [
    ...
    },
    {
      ...
      badge: "PRO",
      badgeColor: void 0
      // will fallback to theme badgeAccent
    },
    {
      icon: BookOpen,
      label: "Notes Index",
      path: "/notes",
      badge: notes.length > 0 ? String(notes.length) : void 0,
    ...
    },
    {
      ...
      label: "Target Board",
      path: "/tasks",
      badge: tasks.filter((t) => t.status !== "completed").length > 0 ? String(tasks.filter((t) => t.status !== "completed").length) : void 0,
  ...
  />
          ...
          {!sidebarExpanded ? null : <motion.div
    initial={{ opacity: 0, x: -10 }}
    animate={{ opacity: 1, x: 0 }}
    exit={{ opacity: 0 }}
  ...
  >
    ...
    /* Navigation Groups */
      ...
      <div className="flex-1 flex flex-col gap-1.5 px-3 overflow-y-auto">
        <div className="mt-1">
          {!sidebarExpanded ? null : <h3 className="px-3 text-[10px] font-mono font-semibold tracking-wider text-zinc-650 uppercase mb-2">
              Workspace Core
            </h3>}
  ...
  >
          ...
          </button> : <div className="flex items-center justify-between p-2.5 rounded-lg bg-zinc-900/30 border border-zinc-900/60 text-xs">
            <div className="flex items-center gap-2">
