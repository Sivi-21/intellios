...
export const Navbar = () => {
  ...
  const getPageTitle = () => {
    switch (location.pathname) {
      case "/":
        return { main: "Architect Studio", sub: "Control Center" };
      case "/chat":
        return { main: "AI Intelligence Node", sub: "Copilot Chat" };
      case "/notes":
        return { main: "Document Index", sub: "Draft Blueprints" };
      case "/tasks":
        return { main: "Target Board", sub: "Sprint Epics" };
      case "/settings":
        return { main: "Workspace Prefs", sub: "System Environment" };
      default:
        return { main: "IntelliOS", sub: "Autonomous Desk" };
    }
  };
  const title = getPageTitle();
  return <header className="sticky top-0 z-40 w-full h-16 bg-zinc-950/80 backdrop-blur-xl border-b border-zinc-900/60 flex items-center justify-between px-6 select-none">
  ...
  />
    ...
    className={`relative p-2 rounded-lg border transition-all ${showNotifications ? "bg-zinc-900 text-zinc-200 border-zinc-700" : "bg-zinc-950/40 hover:bg-zinc-900 text-zinc-400 hover:text-zinc-200 border-zinc-900 hover:border-zinc-800"}`}
  >
  ...
  >
                  ...
                  <div className="overflow-y-auto flex-1 divide-y divide-zinc-900/50">
                    {recentActivities.map((act) => <div key={act.id} className="p-3.5 hover:bg-zinc-900/40 transition-colors flex gap-2.5 text-left">
